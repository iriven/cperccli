...
class PERCParser(object):
    ...
    def __init__(self, data=None, **kwargs) -> None:
        ...
        kwargs.setdefault('specialKeys', ['Controller', 'Model', 'Serial Number', 'Controller Status','SAS Address', 'PCI Address'])
    ...
    def _parse(self, ResponseData: str)->list:
        ...
        for block in Blocks:
            ...
            if stdBlock:
                blockStyle = 'table'
                blockDatas =dict({entry[0] : entry[1] for entry in stdBlock})
                if len(blockDatas) < 1:
                    continue
    ...
    def render(self):
        for item in self.__data:
            ...
            if style == 'table':
                self.__renderer(data=item.get('data'), headers=item.get('header'))
            else:
                self.__vrenderer(data=item.get('data'))
            for i in  range(0, 2)[::-1]:
                print()
